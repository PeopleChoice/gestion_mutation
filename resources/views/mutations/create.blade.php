@extends('layouts.app')
@section('title', 'Nouvelle mutation manuelle')
...
<form method="POST" action="{{ route('mutations.store') }}" id="mutationForm">
    @csrf

    {{-- Etape 1 : Selection parcelle --}}
    <div class="form-section">
        <div class="section-title"><i class="bi bi-1-circle-fill" style="color:#C8A951;"></i> Parcelle concernee</div>

        <div id="parcelleSelectedBox" style="display:{{ $parcelle ? 'block' : 'none' }};">
            <div class="selected-parcelle" style="background:linear-gradient(to right, #fef3c7 0%, #fff8e1 100%); border-color:#d97706;">
                <div class="d-flex justify-content-between align-items-start gap-3">
                    <div>
                        ...
                        <div class="value" id="selectedLot">{{ $parcelle?->numero_lot }}</div>
                        <div class="small mt-1" id="selectedProj">{{ $parcelle?->projet?->nom }}</div>
                    </div>
                    <div class="flex-fill">
                        <div class="label"><i class="bi bi-person-fill" style="color:#d97706;"></i> Précédent propriétaire (sera remplacé)</div>
                        <div class="value" id="selectedProp">{{ $parcelle?->proprietaire?->nom_complet ?? 'Aucun' }}</div>
                        <div class="small" id="selectedCni">
                            @if($parcelle?->proprietaire?->cni_passport)
                                {{ $parcelle->proprietaire->type_piece }} n° {{ $parcelle->proprietaire->cni_passport }}
                            @endif
                            @if($parcelle?->proprietaire?->telephone) · 📞 {{ $parcelle->proprietaire->telephone }} @endif
                        </div>
                    </div>
                ...
                @if($parcelle && !$parcelle->proprietaire)
            ...
            <input type="hidden" name="parcelle_id" id="parcelleId" value="{{ $parcelle?->id }}">
            <input type="hidden" id="parcelleHasOwner" value="{{ $parcelle?->proprietaire ? '1' : '0' }}">
        </div>

        <div id="parcelleSearchBox" style="display:{{ $parcelle ? 'none' : 'block' }};">
                ...
                <div class="col-md-5">
                    ...
                    <select id="projetFilter" class="form-select" style="border-radius:10px;">
                        ...
                        @foreach($projets as $proj)
                            <option value="{{ $proj->id }}">{{ $proj->code }} — {{ $proj->nom }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-7">
            ...
            <div class="row g-3">
                <div class="col-md-4">
                    ...
                    <select name="civilite" class="form-select" style="border-radius:10px;">
                        ...
                        <option value="Société" {{ old('civilite') === 'Société' ? 'selected' : '' }}>Societe</option>
                    </select>
                </div>
                <div class="col-md-4">
                    <label class="form-label small fw-semibold">Prenom / Raison sociale <span class="text-danger">*</span></label>
                    <input type="text" name="prenom" class="form-control" value="{{ old('prenom') }}" style="border-radius:10px;">
                </div>
                <div class="col-md-4">
                    <label class="form-label small fw-semibold">Nom</label>
                    <input type="text" name="nom" class="form-control" value="{{ old('nom') }}" style="border-radius:10px;">
                </div>
                <div class="col-md-3">
                ...
                <div class="col-md-3">
                    ...
                    <input type="text" name="code_paye" id="newCodePaye" class="form-control" value="{{ old('code_paye') }}" placeholder="Ex: SN" style="border-radius:10px;">
                </div>
                <div class="col-md-3">
                    <label class="form-label small fw-semibold">N° pièce <span class="text-danger">*</span></label>
                    <input type="text" name="cni_passport" id="newCniPassport" class="form-control" value="{{ old('cni_passport') }}" style="border-radius:10px;">
                </div>
                <div class="col-md-3">
                    <label class="form-label small fw-semibold">Téléphone</label>
                    <input type="text" name="telephone" class="form-control" value="{{ old('telephone') }}" style="border-radius:10px;">
                </div>
                <div class="col-md-6">
                ...
                <div class="col-md-3">
                    ...
                    <input type="text" name="ninea" class="form-control" value="{{ old('ninea') }}" style="border-radius:10px;">
                </div>
                <div class="col-md-3">
                    <label class="form-label small fw-semibold">Adresse</label>
                    <input type="text" name="adresse" class="form-control" value="{{ old('adresse') }}" style="border-radius:10px;">
                </div>
            </div>
        </div>
    ...
    <div class="form-section">
        ...
        <div class="row g-3">
            <div class="col-md-4">
                ...
                <input type="text" name="numero_notification" class="form-control" value="{{ old('numero_notification') }}" style="border-radius:10px;" placeholder="Ex: 0004857 (auto si vide)">
                @if($dernierNumero)
                    <small class="text-muted">
                        Dernier utilisé : <strong style="font-family:'Courier New',monospace; color:#92400e;">{{ $dernierNumero }}</strong>
                        — prochain : <strong style="font-family:'Courier New',monospace; color:#5D4E37;">{{ str_pad((int) preg_replace('/\D/', '', $dernierNumero) + 1, 7, '0', STR_PAD_LEFT) }}</strong>
                    </small>
                @else
            ...
            <div class="col-md-4">
                ...
                <input type="date" name="date_mutation" class="form-control" value="{{ old('date_mutation', now()->format('Y-m-d')) }}" required style="border-radius:10px;">
            </div>
            <div class="col-md-4">
                <label class="form-label small fw-semibold">Ref. lettre</label>
                <input type="text" name="ref_lettre" class="form-control" value="{{ old('ref_lettre') }}" style="border-radius:10px;" placeholder="Ex: 2025/DGID/123">
            </div>
            <div class="col-12">
                <label class="form-label small fw-semibold">Observation</label>
                <textarea name="observation" class="form-control" rows="2" style="border-radius:10px;" placeholder="Contexte, reference, notes...">{{ old('observation') }}</textarea>
            </div>
        </div>
        ...
        <div class="row g-3">
            <div class="col-md-4">
                ...
                <input type="text" name="demandeur_prenom" class="form-control" value="{{ old('demandeur_prenom') }}" style="border-radius:10px;">
            </div>
            <div class="col-md-4">
                <label class="form-label small fw-semibold">Nom demandeur</label>
                <input type="text" name="demandeur_nom" class="form-control" value="{{ old('demandeur_nom') }}" style="border-radius:10px;">
            </div>
            <div class="col-md-4">
                <label class="form-label small fw-semibold">Téléphone demandeur</label>
                <input type="text" name="demandeur_telephone" class="form-control" value="{{ old('demandeur_telephone') }}" style="border-radius:10px;">
            </div>
        </div>
    </div>
    ...
    <div class="d-flex gap-2 justify-content-end">
        <a href="{{ route('mutations.index') }}" class="btn btn-outline-secondary" style="border-radius:10px;">Annuler</a>
...
(function() {
    ...
    function loadParcelles(q) {
        ...
        let url = '{{ route("mutations.api.parcelles") }}?q=' + encodeURIComponent(q);
    ...
    function loadProps(q) {
        fetch('{{ route("mutations.api.proprietaires") }}?q=' + encodeURIComponent(q))
            .then(r => r.json())
            .then(data => {
                if (!data.length) {
    ...
    document.getElementById('mutationForm').addEventListener('submit', function(e) {
        if (!parcelleIdInput.value) {
            e.preventDefault();
            showToast('Veuillez sélectionner une parcelle.', 'warning');
            return;
        }
        ...
        if (!hasOwner) {
            e.preventDefault();
            showToast('Cette parcelle est vierge (sans propriétaire). Une mutation suppose un changement de propriétaire — utilisez l\'attribution depuis la page projet.', 'warning');
            return;
        }
        // Si mode existant actif, il faut un proprietaire selectionne
        if (tabExisting.classList.contains('active') && !propIdInput.value) {
            e.preventDefault();
            showToast('Veuillez sélectionner un propriétaire existant.', 'warning');
            return;
        }
