@extends('layouts.app')
@section('title', 'Modifier le document - ' . $template->nom)
            ...
            <div class="card">
                ...
                <div class="card-body p-0">
                    ...
                    <div class="section-body" id="sectionInfos">
                        <div class="row g-2 align-items-end">
                            <div class="col-md-4">
                                ...
                                <input type="text" name="nom" class="form-control form-control-sm" value="{{ $template->nom }}" required>
                            </div>
                            <div class="col-md-3">
                                <label class="form-label small fw-bold">Type</label>
                                <input type="text" name="type" class="form-control form-control-sm" value="{{ $template->type }}" required
                            ...
                            <div class="col-md-3">
                                ...
                                <input type="text" name="centre_fiscal" class="form-control form-control-sm" value="{{ $template->centre_fiscal }}">
                            </div>
                            <div class="col-md-2">
                                <label class="form-label small fw-bold">Statut</label>
                                <div class="form-check form-switch mt-1">
                                    <input class="form-check-input" type="checkbox" name="actif" id="actifSwitch" value="1" {{ $template->actif ? 'checked' : '' }}>
                                    <label class="form-check-label small" for="actifSwitch">
                                        <span id="actifLabel">{{ $template->actif ? 'Actif' : 'Inactif' }}</span>
                                    </label>
                                </div>
                            </div>
                            <div class="col-md-12 mt-2">
                                <label class="form-label small fw-bold">Bureau</label>
                                <input type="text" name="bureau" class="form-control form-control-sm" value="{{ $template->bureau }}">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="section-body collapsed" id="sectionEntete">
                        ...
                        <textarea name="entete_html" id="enteteEditor">{!! htmlspecialchars($template->entete_html) !!}</textarea>
                    </div>
                    ...
                    <div class="section-body" id="sectionCorps">
                        <textarea name="corps_html" id="corpsEditor">{!! htmlspecialchars($template->corps_html) !!}</textarea>
                    </div>
                    ...
                    <div class="section-body" id="sectionPied">
                        <textarea name="pied_html" id="piedEditor">{!! htmlspecialchars($template->pied_html) !!}</textarea>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="preview-panel">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center py-2">
                    ...
                    <a href="{{ route('templates.preview', $template) }}" class="btn btn-sm btn-outline-secondary" target="_blank">
                        <i class="bi bi-box-arrow-up-right"></i> Plein écran
                    </a>
                </div>
                <div class="preview-frame">
                    <iframe id="previewFrame" src="{{ route('templates.preview', $template) }}"></iframe>
                </div>
            </div>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.rafraichirApercu = function() {
        ...
        .then(() => {
            document.getElementById('previewFrame').src = '{{ route("templates.preview", $template) }}?' + Date.now();
        })
